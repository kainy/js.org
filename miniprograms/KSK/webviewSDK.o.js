    ...
    (function (global) {
      ...
      */
      ...
      webview.shareTitle = function (data = {
        title: webview.cfgWebview.title, // 分享小卡片的标题
        extend: '', // 小卡片传递参数，可在目标页中通过 params().extend 访问
        webviewUrl: webview.cfgWebview.link // 分享页面的 URL，不传则取当前页 URL
      }) {
       // console.log(data)
      ...
      webview.shareTitle(); // 默认执行一次
      wx.ready(function () {
        console.warn('wx.ready')
        var obj = { 
          title: webview.cfgWebview.title, // 分享标题
          desc: webview.cfgWebview.desc, // 分享描述
          link: webview.cfgWebview.link, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
